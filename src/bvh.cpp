...
BVH_Box::BVH_Box(const std::string &fname, mat_pointer material) {
  // Initialize Loader
  objl::Loader Loader;
  ...
  for (int i = 0; i < mesh.Indices.size(); i += 3) {
    ...
    obj_pointer triangle = std::make_shared<Triangle>(
        Vec3(v1.X, v1.Y, v1.Z), Vec3(v2.X, v2.Y, v2.Z), Vec3(v3.X, v3.Y, v3.Z),
        material);
  ...
  Vec3 min = Vec3(v.X, v.Y, v.Z);
  Vec3 max = Vec3(v.X, v.Y, v.Z);
  for (int i = 0; i < mesh.Vertices.size(); i += 3) {
    auto &v1 = mesh.Vertices[i].Position;
    min = min_v_P(min, v1.X, v1.Y, v1.Z);
    max = max_v_P(min, v1.X, v1.Y, v1.Z);
  }

  std::cout << min << max << triangles.size() << std::endl;
}
