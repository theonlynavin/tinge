 ...
 * @par vertical_fov : It is the extent of observable world expressend in terms
...
Ray Camera ::generate_ray(float u, float v, Random &random_gen) {
    ...
    // Converting the ray from camera frame to world frame and then normalizing
    // as well
 ...
 * @par "from" is the origin of camera and "at" is the final point to be focused
 * upon
...
void Camera::look_at(Vec3 from, Vec3 to) {
    ...
    Vec3 axis =
        cross(A, B)
            .normalized(); // axis of rotaion , which we got through cross
                           // multiplication of initial and final vectors
    float dp = dot(A, B);  // angle through which vector need to be rotated
    float angle = std::acos(dp);

    Mat3 K;
    Mat3 I;
    K.m[0][0] = 0; // rotation matrix , to know more details read about
    ...
    float pitch =
        -std::asin(K.m[2][0]); // Extraction of rotation angle needed about
                               // various axes using the rotation matrix
    float yaw = std::atan2(
        K.m[1][0], K.m[1][1]); // For more details , google pitch ,yaw and roll
